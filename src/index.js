...
io.on('connection', (socket)=>{//runs for each connection
    ...
    socket.on('joinChat', ({username, room}) => {
        console.log(
            username, room, 'kya scene'
        )
        socket.join(room)
        socket.emit('message', generateMessage('Welcome!')) //emits to that particular connection
     socket.broadcast.to(room).emit('message', generateMessage(`${username} has joined ${room}`))//emits to every other connection
    })
